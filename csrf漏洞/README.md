...
| 绑非会话 Cookie | csrfKey 不绑 session | CRLF 写 Cookie |
...
| 坑 | 原因 | 解决 |
|----|------|------|
| action/src 带 `[url](url)` | 从聊天窗口复制 URL，Markdown 格式被带入 | 手动输入纯 URL |
| HTML 标签前有 `\` | 从聊天复制时反斜杠被带入 | 手动输入或检查 |
| email 用 `&#64;` 实体编码 | 从某处复制时被编码 | 直接写明文 `@` |
| Burp PoC 缺参数 | 生成器只出模板，不自动补全 | 手动加 hidden input |
| GET 参数放 Body | GET 不读 Body | 参数移到 URL `?` 后面 |
| csrfKey 值为空 | 只写了 `%3b` 没填值 | 填实际 csrfKey |
| img src 用 Exploit Server 域名 | Cookie 写在错误域名下 | 用靶场域名 |
| Token 过期 | 拿到后隔太久才用 | 快速完成 Store + Deliver |
| 同时改 Cookie 和 Token | 等于还是用 A 的会话 | 只改 Token，Cookie 不动 |